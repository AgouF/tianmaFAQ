...
2.  **Integration and Configuration**
    *   **Add an Accelerated Domain**: In the CDN service provider's console, add the domain name you want to accelerate (e.g., `static.yourwebsite.com`). This is often referred to as the "accelerated domain."
...
After completing the above configuration and waiting for DNS propagation (usually from a few minutes to several hours), your website resources will begin to be distributed via the CDN. You can use online CDN detection tools, enter your accelerated domain, and check if resources are indeed loading from CDN nodes.
...
*   **Cost Control**: Be aware of the CDN billing model (usually based on traffic or peak bandwidth), set reasonable alert thresholds to avoid unexpected costs due to traffic surges.
...
### Will configuring a CNAME record for CDN affect other services like email?
