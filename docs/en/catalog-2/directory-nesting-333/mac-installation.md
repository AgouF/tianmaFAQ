...
1. How do you want to provide the gateway token? How to save the gateway Token Directly press Enter to select the default.
...
1. Permission Management Open permissions and configure all permissions related to messages.

1. At this point, you can continue setting up in the Openclaw terminal.

1. After setting up, click on Events & Callbacks, select WSS long connection. After selecting, add events and callback events, search for messages, add all related to messages. The same for callbacks. This way you can converse with OPenclaw. Remember to publish the application.
...
| Option | Meaning |
| --- | --- |
| **Allowlist** | Only reply in specified groups |
| Open | Reply in all groups (must @ the bot) |
| Disabled | Do not reply in group chats at all |

1. I select the first one: Group chat allowlist (chat_ids). Open the APP on the mobile device, select the group, and find ocxxxx at the very bottom of the group.

1. Then select Finished (Done) to confirm the configuration.

1. Configure DM access policies now? (default: pairing) Configure private chat (DM = Direct Message) access policy now? Select Yes.

1. Feishu DM policy Private chat policy Select the first one: Pairing (only private chat after pairing).

1. Search provider Which search engine does OpenClaw use to search the internet for information? Just skip it.
